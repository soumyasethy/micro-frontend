...
export const enhanceLimit: ActionFunction<EnhanceLimitPayload> = async (
    ...
    console.log('applicationType => ', action.payload.applicationType)

    const response = await network.post(
        `${api.getLimitModificationApplicationForAccount}${borrowerAccountId}`,
        {
            creditApplicationType: action.payload.applicationType,
        },
        {
...
export const getURL: ActionFunction<manageLimitPayload> = async (
    ...
    if (response.status === 200) {
        ...
        showPopup({
            ...
            subTitle:
                'Account holding details have been sent to your email address.',
            ctaLabel: 'Continue',
            ctaAction: {
                type: ACTION.MENU,
