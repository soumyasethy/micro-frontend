...
export const triggerAction: ActionFunction<AadharInitPayload> = async (
  ...
  const response = await network.get(
    `${api.documentValidate}${applicationId}`,
...
> = async (action, _datastore, { network, setDatastore }): Promise<any> => {
  if (action.payload.value) {
    ...
    await setDatastore(ROUTE.KYC_DOCUMENT_UPLOAD, action.payload.widgetID, <
      DocumentPickerProps
    >{
  ...
  if (
    validation.isDropDownSelected &&
    validation.isFrontDocUploaded &&
    validation.isBackDocUploaded
  ) {
    await setDatastore(ROUTE.KYC_DOCUMENT_UPLOAD, "continue", <ButtonProps>{
...
export const onSelectDocumentType: ActionFunction<DropDownPayload> = async (
  action,
  _datastore,
  { network, removeWidgets, appendWidgets, setDatastore }
): Promise<any> => {
  validation.isDropDownSelected = true;
