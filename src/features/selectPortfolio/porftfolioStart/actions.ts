...
import { ACTION, AssetsPayload, EditItemPayload } from "./types";
...
export const onSkip: ActionFunction<AssetsPayload> = async (
  action,
  _datastore,
  { navigate, ...props }
): Promise<any> => {
...
export const onCopy: ActionFunction<{}> = async (
  action,
  _datastore,
  { network, clipboard, setDatastore, ...props }
): Promise<any> => {
  ...
  clipboard.set(link);
  await setDatastore(ROUTE.PORTFOLOIO_START, "header", <HeaderProps>{
...
export const onShare: ActionFunction<AssetsPayload> = async (
  action,
  _datastore,
  { network, navigate, ...props }
): Promise<any> => {
  const applicationId = await SharedPropsService.getApplicationId();
